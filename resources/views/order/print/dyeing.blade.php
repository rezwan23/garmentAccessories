...
@section('content')
    ...
    <div id="print" style="width:100%;margin:0 auto;">
        ...
        <table border="0" width="100%">
            ...
            <td width="35%">
                <div style="width:100%; height:180px;border:2px solid #000">
                    <h2 style="text-align: center;border-bottom:2px dashed">Dyeing Copy</h2>
                    {{--<h3 style="text-align: center; margin:0px;">{{auth()->user()->company->name}}</h3>--}}
                    <hr>
                    <h2>Date : {{$order->order_date}}</h2>
                </div>
            </td>
        </table>
        ...
        <table border="0" width="100%" style="margin-top:10px">
            @if($order->dyeingCompany)
                <tr>
                    <td width="25%"><h4 style="text-transform: uppercase;margin: 0;padding: 0;text-transform:uppercase">DYEING COMPANY : </h4></td>
                    <td width="75%"><h4 style="text-transform: uppercase;margin: 0;padding: 0;"><span style="border-bottom:1px solid #000;width: 100%;display: block;">{{$order->dyeingCompany->name}}</span></h4></td>
                </tr>
            <tr>
                <td height="20px"></td>
            </tr>
                <tr>
                    <td width="25%"><h4 style="text-transform: uppercase;margin: 0;padding: 0;text-transform:uppercase">DYEING DELIVERY DATE : </h4></td>
                    <td width="75%"><h4 style="text-transform: uppercase;margin: 0;padding: 0;"><span style="border-bottom:1px solid #000;width: 100%;display: block;">{{$order->dyeing_delivery_date}}</span></h4></td>
                </tr>
                <tr>
                    <td height="20px"></td>
                </tr>
            <tr>
                <td width="25%"><h4 style="text-transform: uppercase;margin: 0;padding: 0;text-transform:uppercase">DYEING COMPANY ADDRESS : </h4></td>
                <td width="75%"><h4 style="text-transform: uppercase;margin: 0;padding: 0;"><span style="border-bottom:1px solid #000;width: 100%;display: block;">{{$order->dyeingCompany->address}}</span></h4></td>
            </tr>
            <tr>
                <td height="20px"></td>
            </tr>
            <tr>
                ...
                <td width="75%"><h4 style="text-transform: uppercase;margin: 0;padding: 0;"><span style="border-bottom:1px solid #000;width: 100%;display: block;">{{auth()->user()->company->dyeing_delivery_place}}</span></h4></td>
            </tr>
            @endif
            <tr>
                <td height="20px"></td>
        ...
        <div class="info" style="overflow: hidden;">
            <div style="margin-top:10px;width: 40%;margin-right: 5%; float: left;">
                ...
                <table border="1" style="width: 100%;border-collapse: collapse;" class="item_commercial">
                    ...
                    @foreach($order->items as $item)
                        <tr>
                            <td>{{$item->item->name}} - {{$item->size}} ({{$item->quantity.' '.$item->item->unit->name}})</td>
            ...
            <div style="margin-top:10px;width: 55%; float:left; overflow: hidden;">
                @foreach($order->items as $item)
                    <table border="1" width="100%" style="border-collapse: collapse;font-weight: 800;" class="raw" style="margin-bottom: 4px;">
                        <thead>
                        <tr>
                        ...
                        @foreach($item->requirements as $requirement)
                            <tr>
                                <td><strong>{{$requirement->accessory->name}}</strong></td>
                                <td><strong>{{$requirement->quantity}} {{$requirement->accessory->unit->name}}</strong></td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
        ...
        <div>
            <p style="margin-top:10px;"><strong>note-pls mention the style/ref number of delivery challan & color number also.</strong>
                <br>
                test -azo free, allergenic disperse dyes, all color fastness, apeo.
            </p>
        </div>
