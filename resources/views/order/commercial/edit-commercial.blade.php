...
@section('title', 'Edit Commercial Details')
...
{{--                                <input required="" class="form-control" value="{{$order->order_date}}" id="demoDate" name="order_date" autocomplete="off" type="text" placeholder="Select Order Date">--}}
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <form action="{{route('commercial.update', $order)}}" method="post">
                                    @csrf
                                            ...
                                            @foreach($order->commercial->commercials as $commercial)
                                                <tr>
                                                    <td>
                                                        ...
                                                        <input disabled="" required type="text" value="{{$commercial->orderedItem->item->name}}" class="form-control">
                                                    </td>
                                                    <td><input required step=".0001" type="number" id="unit_{{$loop->index}}" onkeyup="countTotal(this.id)" min="0" class="form-control" value="{{$commercial->unit_price}}" name="unit_price[{{$commercial->id}}][]"></td>
                                                    <td><input required type="number" id="quantity_{{$loop->index}}" onkeyup="countTotal(this.id)" min="0" class="form-control" value="{{$commercial->quantity}}" name="quantity[{{$commercial->id}}][]"></td>
                                                    <td><input required type="text" step=".0001" readonly id="total_{{$loop->index}}" class="form-control total_price" value="{{$commercial->unit_price*$commercial->quantity}}"></td>
                                                </tr>
                                            @endforeach
                                            </tbody>
                                        </table>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                <table class="table table-bordered">
                                                    <tr>
                                                        <th>Total Amount</th>
                                                        <td><input step=".0001" required type="text" value="{{$order->commercial->total_amount}}" name="total_amount" class="form-control" id="total_amount"></td>
                                                    </tr>
                                                </table>
                                            </div>
                                        </div>
