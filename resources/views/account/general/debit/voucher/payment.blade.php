...
@section('content')
    <div class="tile">
        <div class="tile-title-w-btn">
            ...
            <p>
                <a href="{!! route('accounts.debit-vouchers.index') !!}" class="btn btn-primary">View Payment</a>
            </p>
        </div>
        <div class="tile-body" id="app-container">
            ...
            <div v-show="!isEmpty(data)" style="display: none">
                ...
                <form v-if="!isEmpty(data)" :action="`/accounts/debit-vouchers/${id}/paid`" method="post" id="payment">
                    @csrf
                    <hr>
...
@section('footer')
    ...
    <script>
        ...
        $select.select2().on('change', function () {
            ...
            if(select.val()){
                $.getJSON('/accounts/debit-vouchers/payment/'+select.val(), function (data) {
                    app.data = data.data;
                    if(!app.isOpen){
                        app.isOpen = true;
                        setTimeout(function () {
                            $('#payment').transmitData({
                                successCallback: function (response) {
                                    ...
                                    setTimeout(function () {
                                        location.href = '/accounts/debit-voucher-payments/'+response.payment.id+'/voucher?auto-print';
                                    }, 500);
                                }
                            });
